...
export function ExpenseProvider({ children }) {
  ...
  const createAbortController = (requestId) => {
    ...
    if (abortControllers.current[requestId]) {
      try {
        ...
        console.log(`Error aborting previous request ${requestId}:`, err);
      }
    }
    ...
    // Create a cleanup function to remove the controller when done
    const cleanup = () => {
      if (abortControllers.current[requestId] === controller) {
  ...
  const fetchExpenseWithParticipants = useCallback(async (expenseId) => {
    ...
    const { controller, cleanup } = createAbortController(requestId);
  ...
  const fetchData = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      try {
        ...
        console.error('Exception fetching trips:', tripErr);
      ...
      try {
        ...
        console.error('Exception fetching expenses:', expenseErr);
        // Continue with other fetches even if this one fails
      }
      ...
      try {
        ...
        console.error('Exception fetching expense participants:', participantsErr);
        // Continue with other fetches even if this one fails
      }
      ...
      try {
        ...
        console.error('Exception fetching users:', usersErr);
        // Continue even if this fetch fails
      }
      
      // Mark data as fetched - only if we got here without a major error
  ...
  const refreshDataOnce = useCallback(async () => {
    ...
    // Create a unique request ID for this refresh
    const requestId = `refreshDataOnce_${Date.now()}`;
    ...
    try {
      ...
      // Call fetchData with force refresh
      ...
      console.error('Error in refreshDataOnce:', error);
      return false;
    } finally {
      delete pendingRequests.current[requestId];
    }
  ...
  // Get active trips (not marked as settled)
  const getActiveTrips = useCallback(() => {
    return trips.filter(trip => !trip.is_settled);
  }, [trips]);

  // Get settled trips
  const getSettledTrips = useCallback(() => {
    return trips.filter(trip => trip.is_settled);
  }, [trips]);

  // Calculate settlements for a specific trip
  const calculateTripWiseSettlements = useCallback((tripId) => {
    try {
      ...
      // Initialize debt matrix for all friends
      ...
      // Process each expense in this trip to determine who owes what to whom
      ...
      console.error(`Error calculating settlements for trip ${tripId}:`, error);
      return [];
    }
  }, [expenses, expenseParticipants, friends]);

  // Calculate settlements for all active trips
  const calculateAllTripSettlements = useCallback(() => {
    try {
      const activeTrips = getActiveTrips();
      let allSettlements = [];
      
      // Calculate settlements for each active trip
      activeTrips.forEach(trip => {
        const tripSettlements = calculateTripWiseSettlements(trip.id);
        allSettlements = [...allSettlements, ...tripSettlements];
      });
      
      // Combine settlements between the same people
      const combinedSettlements = {};
      
      allSettlements.forEach(settlement => {
        const key = `${settlement.from}_${settlement.to}`;
        
        if (!combinedSettlements[key]) {
          combinedSettlements[key] = {
        ...
        combinedSettlements[key].amount += settlement.amount;
        
        // Track which trips this settlement is part of
        if (!combinedSettlements[key].trips.includes(settlement.tripId)) {
          combinedSettlements[key].trips.push(settlement.tripId);
        }
      });
      
      // Convert back to array and filter out zero amounts
      return Object.values(combinedSettlements)
        .filter(settlement => settlement.amount > 0.01)
        .sort((a, b) => b.amount - a.amount); // Sort by amount (highest first)
    } catch (error) {
      console.error("Error calculating all trip settlements:", error);
      return [];
    }
  }, [getActiveTrips, calculateTripWiseSettlements]);

  // Calculate smart settlements for active trips only
  const calculateActiveSmartSettlements = useCallback(() => {
    try {
      // Get only active trips
      const activeTrips = getActiveTrips();
      ...
      // Get only expenses from active trips
      const activeTripIds = activeTrips.map(trip => trip.id);
      const activeExpenses = expenses.filter(expense => 
        expense.trip_id && activeTripIds.includes(expense.trip_id)
      );
      
      // Step 1: Calculate the net balance for each person (only for active trips)
      const balances = {};
      ...
      activeExpenses.forEach(expense => {
        ...
        participants.forEach(participant => {
          if (participant.user_id && participant.user_id !== payer) {
            ...
            // Add to the debt that this participant owes to the payer
            if (balances[participant.user_id] !== undefined) {
        ...
        if (balances[payer] !== undefined) {
          balances[payer] += parseFloat(expense.amount);
        }
      });
      ...
      while (debtors.length > 0 && creditors.length > 0) {
        ...
        if (amount > 0.01) {
          smartSettlements.push({
            ...
            to: creditor.id,
            amount: amount,
            isActive: true // Flag to indicate this is from active trips only
          });
        }
      ...
      console.error("Error calculating active smart settlements:", error);
      return [];
    }
  }, [getActiveTrips, expenses, expenseParticipants, friends]);
  ...
  const getTripById = useCallback(async (tripId) => {
    ...
    try {
      ...
      // especially for newly created trips that might not be in the local state yet
      ...
      if (tripError) {
        if (tripError.code === 'PGRST116') {
          ...
          // If we have it locally but not in DB, return the local version as a fallback
          if (localTrip) {
            console.log("Trip not found in database but exists locally, using local data:", localTrip);
            return localTrip;
          }
          return null;
        }
        ...
        // If we have it locally but DB query failed, return the local version as a fallback
        if (localTrip) {
          console.log("Error fetching from database but trip exists locally, using local data:", localTrip);
          return localTrip;
        }
        return null; 
      }
      ...
      if (!tripData) {
        ...
        // If we have it locally but not in DB response, return the local version as a fallback
        if (localTrip) {
          console.log("Trip not in database response but exists locally, using local data:", localTrip);
          return localTrip;
        }
        return null;
      }
      ...
      // Update our local state with the latest data from the database
      if (!localTrip) {
        setTrips(prevTrips => [...prevTrips, tripData]);
      } else if (JSON.stringify(localTrip) !== JSON.stringify(tripData)) {
        // Update the local trip if it's different from what we got from the database
        setTrips(prevTrips => 
          prevTrips.map(trip => 
            trip.id === tripId ? tripData : trip
          )
        );
      }
  ...
  const markTripAsSettled = useCallback(async (tripId, isSettled = true) => {
    if (!tripId) {
      throw new Error("Trip ID is required");
    }
    
    const requestId = `markTripAsSettled_${tripId}`;
    const { controller, cleanup } = createAbortController(requestId);
    const signal = controller.signal;
    ...
    try {
      ...
      if (error) {
        console.error('Error marking trip as settled:', error);
        throw new Error(`Failed to mark trip as settled: ${error.message}`);
      }
      
      // Update local state
      setTrips(prev => prev.map(trip => 
        trip.id === tripId ? { ...trip, is_settled: isSettled } : trip
      ));
      ...
      // Only log error if not aborted
      if (err.name !== 'AbortError') {
        console.error('Error marking trip as settled:', err.message || JSON.stringify(err));
        throw err;
      }
      return null;
    } finally {
  ...
  // Toggle trip settlement status
  const toggleTripSettlementStatus = useCallback(async (tripId) => {
    if (!tripId) {
      throw new Error("Trip ID is required");
    }
    
    // Find the trip in local state to get current status
    ...
    // Toggle the status
    return markTripAsSettled(tripId, !trip.is_settled);
  }, [trips, markTripAsSettled]);
    ...
    try {
      ...
      expenses.forEach(expense => {
        ...
        if (expense.paid_by) {
          ...
          balances[expense.paid_by] = (balances[expense.paid_by] || 0) + paidForOthers;
          console.log(`After payer ${expense.paid_by} added ${paidForOthers} for expense "${expense.description}":`, {...balances});
        }
      });
  ...
  // Calculate balances excluding settled trips
  const calculateBalancesExcludingSettled = useCallback(() => {
    const balances = {};
    ...
    // Filter expenses to only include those from active trips
    const activeExpenses = expenses.filter(expense => 
      expense.trip_id && activeTripIds.includes(expense.trip_id)
    );

    // Calculate net balance for each friend using only active expenses
    activeExpenses.forEach(expense => {
      // Find participants for this expense
      const participants = expenseParticipants.filter(p => p.expense_id === expense.id);
    ...
    return balances;
  }, [friends, expenses, expenseParticipants, getActiveTrips]);
  ...
  const addTrip = useCallback(async (trip) => {
    ...
    try {
      ...
      // Add the new trip to our local state immediately
      const newTripWithMembers = {
        ...data[0],
        members: members || []
      };
      ...
      setTrips(prevTrips => {
        // Check if the trip already exists in the state
        const exists = prevTrips.some(t => t.id === tripId);
        if (exists) {
          // Update the existing trip
          return prevTrips.map(t => t.id === tripId ? newTripWithMembers : t);
        } else {
          // Add the new trip
          return [...prevTrips, newTripWithMembers];
        }
      });
      
      // Force a refresh of the data to ensure everything is in sync
      refreshDataOnce();
  ...
  const addExpense = useCallback(async (expense) => {
    ...
    try {
      ...
      const { participants, ...expenseData } = expense;
      
      // Remove date field as it doesn't exist in the database schema
      const { date, ...validExpenseData } = expenseData;
      
      // Create expense object
      const newExpense = {
        id: uuidv4(),
        ...validExpenseData,
        trip_id: validExpenseData.tripId, // Map tripId to trip_id for database
        paid_by: validExpenseData.paidBy, // Map paidBy to paid_by for database
      };
      
      // Remove fields that don't match the database schema
      delete newExpense.tripId;
      delete newExpense.paidBy;
      ...
      if (participants && participants.length > 0) {
        ...
        console.log("Adding expense participants:", participantRecords);
  ...
  const resetSettlementStatus = useCallback(() => {
    try {
      // Find all localStorage keys that start with 'settlement_'
      const keysToRemove = [];
      ...
      // Remove all settlement status keys
      ...
      console.error('Error resetting settlement status:', err);
      return false;
    }
  }, []);

  // Mark a settlement as settled
  const markSettlementAsSettled = useCallback(async (settlementId, isSettled = true, type = 'regular') => {
    try {
      // Since settlements aren't stored in the database directly, we'll store them in localStorage
      // This is a simple solution for this feature - in a real app, you might want to store this in the database
      const storageKey = `settlement_${type}_${settlementId}`;
      
      if (isSettled) {
        localStorage.setItem(storageKey, 'settled');
      } else {
  ...
  // Check if a settlement is settled
  const isSettlementSettled = useCallback((settlementId, type = 'regular') => {
    try {
      const storageKey = `settlement_${type}_${settlementId}`;
      return localStorage.getItem(storageKey) === 'settled';
    } catch (err) {
      console.error('Error checking if settlement is settled:', err);
      return false;
    }
  }, []);
